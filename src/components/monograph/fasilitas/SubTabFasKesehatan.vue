          ...
          <div class="content vscroll" style="max-height: 600px">
            ...
            <div v-else>
              <div class="container">
                <div class="row">
                  <ListFasilitas
                    v-for="(data, idx) in state.fasKesehatan"
                    :key="idx"
...
const state = reactive({
  fasKesehatan: [] as DataDashboard[],
});

async function getFaskes() {
  loadingState.value = "false";
  await axios
    .get(`${urlApi}dashboard/${kodeWilayahStore.kode?.kode}/sarana_kesehatan`)
    .then(({ data }) => {
      if (data.status == "success") {
        data.datas.forEach((element) => {
          if (element.kategori_variabel == "jumlah_infrastruktur_kesehatan") {
            state.fasKesehatan.push(element);
          }
        });
