          ...
          <div class="content vscroll" style="max-height: 600px">
            ...
            <div v-else>
              <div class="container">
                <div class="row">
                  <ListFasilitas
                    v-for="(data, idx) in state.fasEkonomi"
                    :key="idx"
...
const state = reactive({
  fasEkonomi: [] as DataDashboard[],
});

async function getFasEkonomi() {
  loadingState.value = "false";
  await axios
    .get(`${urlApi}dashboard/${kodeWilayahStore.kode?.kode}/sarana_ekonomi`)
    .then(({ data }) => {
      if (data.status == "success") {
        data.datas.forEach((element) => {
          if (element.kategori_variabel == "jumlah_infrastruktur_ekonomi") {
