          ...
          <div class="content vscroll" style="max-height: 600px">
            ...
            <div v-else>
              <div class="container">
                <div class="row">
                  <ListFasilitas
                    v-for="(data, idx) in state.fasInfokom"
                    :key="idx"
...
const state = reactive({
  fasInfokom: [] as DataDashboard[],
});

async function getFasInfokom() {
  loadingState.value = "false";
  await axios
    .get(
      `${urlApi}dashboard/${kodeWilayahStore.kode?.kode}/sarana_komunikasi_informasi`
    )
    .then(({ data }) => {
