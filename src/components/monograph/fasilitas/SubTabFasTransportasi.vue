          ...
          <div class="content vscroll" style="max-height: 600px">
            ...
            <div v-else>
              <div class="container">
                <div class="row">
                  <ListFasilitas
                    v-for="(data, idx) in state.fasTransportasi"
                    :key="idx"
...
const state = reactive({
  fasTransportasi: [] as DataDashboard[],
});

async function getFasTransportasi() {
  loadingState.value = "false";
  await axios
    .get(
      `${urlApi}dashboard/${kodeWilayahStore.kode?.kode}/sarana_transportasi`
    )
    .then(({ data }) => {
